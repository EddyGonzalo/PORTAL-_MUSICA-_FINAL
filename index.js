...
// Agregar evento clic a cada elemento song-play
document.querySelectorAll('.song-play').forEach((el) => {
  el.addEventListener('click', (event) => {
    // Obtener el id de la canción de la clase única del elemento song-play
    let songId = event.target.classList[3];
    // Actualizar index, posterimg, Title y download_music con la información de la canción correspondiente
    index = songId;
    posterimg.src = event.target.parentNode.querySelector('img').src;
    Title.innerText = event.target.parentNode.nextElementSibling.querySelector('h5').innerText;
    download_music.href = 'path/to/download/' + songId + '.mp3';
    download_music.download = event.target.parentNode.nextElementSibling.querySelector('h5').innerText + '.mp3';
  });
});
